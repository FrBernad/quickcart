...
def test_get_products(monkeypatch, test_shopping_cart_service):
    mock_products = [mock_shopping_cart_product1, mock_shopping_cart_product2]
...
def test_checkout(monkeypatch, test_shopping_cart_service):
    ...
    shopping_cart_dao_mock.get_products = MagicMock(
        spec=mock_get_products, return_value=mock_products
    )
...
def test_checkout_product_out_of_stock_exception(
    monkeypatch, test_shopping_cart_service
):
    mock_shopping_cart_product2_with_huge_quantity = copy.deepcopy(
        mock_shopping_cart_product2
    )

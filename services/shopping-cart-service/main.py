...
def update_product_quantity_from_shopping_cart(user_id,product_id):
    ...
    return f"Update the shopping cart of the user with id {user_id}. The quantity of the product {product_id} is now ${quantity}",200
...
def delete_product_from_shopping_cart(user_id,product_id):
    return f"Deleting product with id {product_id} from the shopping cart of the user with id {user_id}",200
